...
class SLPPro {
    ...
    function admin_init(){
        ...
        // Add Locations
        //
        add_action('slp_addlocations_with_csv', array($this,'bulk_upload_processing'));
    ...
    function filter_MapResults_LocationAddSettings($HTML) {
        $HTML .=
            ...
            $this->plugin->helper->CreateCheckboxDiv(
                '_show_tags',
                __('Show Tags In Output','csa-slp-pro'),
                __('Show the tags in the location output table and bubble.', 'csa-slp-pro')
                );
    ...
    function filter_MapSettings_Search_FeaturesAddSettings($HTML) {
        $HTML .=
            ...
            $this->plugin->helper->CreateCheckboxDiv(
                ...
                __('Hide radius selection','csa-slp-pro'),
                __('Hides the radius selection from the user, the default radius will be used.', 'csa-slp-pro'),
                SLPLUS_PREFIX
                ) .
            $this->plugin->helper->CreateCheckboxDiv(
                '_hide_address_entry',
                __('Hide address entry box','csa-slp-pro'),
                __('Hides the address entry box from the user.', 'csa-slp-pro'),
                SLPLUS_PREFIX
                ) .

            $this->plugin->helper->CreateCheckboxDiv(
                '_use_location_sensor',
                __('Use location sensor', 'csa-slp-pro'),
                __('This turns on the location sensor (GPS) to set the default search address.  This can be slow to load and customers are prompted whether or not to allow location sensing.', 'csa-slp-pro'),
                SLPLUS_PREFIX
                ) .

            $this->plugin->helper->CreateCheckboxDiv(
                'sl_use_city_search',
                __('Show City Pulldown','csa-slp-pro'),
                __('Displays the city pulldown on the search form. It is built from the unique city names in your location list.','csa-slp-pro'),
                ''
                ) .

            $this->plugin->helper->CreateCheckboxDiv(
                'sl_use_country_search',
                __('Show Country Pulldown','csa-slp-pro'),
                __('Displays the country pulldown on the search form. It is built from the unique country names in your location list.','csa-slp-pro'),
                ''
                ) .

            $this->plugin->helper->CreateCheckboxDiv(
                'slplus_show_state_pd',
                __('Show State Pulldown','csa-slp-pro'),
                __('Displays the state pulldown on the search form. It is built from the unique state names in your location list.','csa-slp-pro'),
                ''
                ) .

            $this->plugin->helper->CreateCheckboxDiv(
                '_disable_search',
                __('Hide Find Locations button','csa-slp-pro'),
                __('Remove the "Find Locations" button from the search form.', 'csa-slp-pro'),
                SLPLUS_PREFIX
                );
    ...
    function filter_MapSettings_SearchLabel_AddSettings($HTML) {
        $HTML .=
            ...
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                '_state_pd_label',
                __('State Label', 'csa-slp-pro'),
                __('Search form label to prefix the state selector.','csa-slp-pro')
                ).
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                '_find_button_label',
                __('Find Button', 'csa-slp-pro'),
                __('The label on the find button, if text mode is selected.','csa-slp-pro'),
                SLPLUS_PREFIX,
                __('Find Locations','csa-slp-pro')
                );
        return $HTML;
    }
    ...
    function bulk_upload_form($HTML) {
        return ( $HTML .
                    ...
                    '<h2>'.__('Bulk Upload', 'csa-slp-pro').'</h2>'.
                    ...
                    $this->plugin->helper->CreateCheckboxDiv(
                        ...
                        __('Skip First Line','csa-slp-pro'),
                        __('Skip the first line of the import file.','csa-slp-pro'),
                        SLPLUS_PREFIX,
                        false,
                        0
                    ).
                    $this->plugin->helper->CreateCheckboxDiv(
                        ...
                        __('Skip Duplicates','csa-slp-pro'),
                        __('Checks the name, street, city, state, zip, and country for duplicate entries.  Skips if already in database.  Takes longer to load locations.','csa-slp-pro'),
                        SLPLUS_PREFIX,
                        false,
                        0
                    ).

                    "<div class='form_entry'><input type='submit' value='".__('Upload Locations', 'csa-slp-pro')."' class='button-primary'></div>".
                    '</div>' .
                    '</div>'
                );
    }
    ...
    function filter_AddFieldsToEditForm($theHTML) {
        $theHTML .=
            ...
            $this->plugin->helper->create_SubheadingLabel(__('Pro Pack','csa-slp-pro'))
            ;
    ...
    function filter_AddOurAdminSlug($slugs) {
        return array_merge($slugs,
                array(
                    'store-locator-plus_page_slp_propack',
    ...
    function filter_ManageLocations_ShowUncoded($where) {
        ...
        return $where .
                  " sl_latitude NOT REGEXP '^[0-9]|-' or sl_longitude NOT REGEXP '^[0-9]|-' or ".
                  " sl_latitude IS NULL or sl_longitude IS NULL"
                ;
    }
    ...
    function filter_MapFeatures_AddSettings($html) {
        return
            ...
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                'sl_starting_image',
                __('Starting Image','csa-slp-pro'),
                __('If set, this image will be displayed until a search is performed.  Enter the full URL for the image.','csa-slp-pro'),
                ''
                ) .
            $this->plugin->helper->CreateCheckboxDiv(
                '_disable_initialdirectory',
                __('Disable Initial Directory','csa-slp-pro'),
                __('Do not display the listings under the map when "immediately show locations" is checked.', 'csa-slp-pro')
                )
            ;
    }
    ...
    function filter_MapSettings_AddTagsBox($html) {
        $html .=
            ...
            '<h2>'.__('Pro Pack Tags', 'csa-slp-pro').'</h2>' .
            '<div class="section_column_content">' .
            $this->plugin->helper->CreateCheckboxDiv(
                '_show_tag_search',
                __('Tag Input','csa-slp-pro'),
                __('Show the tag entry box on the search form.', 'csa-slp-pro')
                ).
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                '_search_tag_label',
                __('Tags', 'csa-slp-pro'),
            ...
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                    ...
                    __('Preselected Tag Searches', 'csa-slp-pro'),
                    __("Enter a comma (,) separated list of tags to show in the search pulldown, mark the default selection with parenthesis '( )'. This is a default setting that can be overriden on each page within the shortcode.",'csa-slp-pro')
                    ).
            $this->plugin->helper->CreateCheckboxDiv(
                '_show_tag_any',
                __('Add "any" to tags pulldown','csa-slp-pro'),
                __('Add an "any" selection on the tag pulldown list thus allowing the user to show all locations in the area, not just those matching a selected tag.', 'csa-slp-pro')
                ).
            $this->plugin->AdminUI->MapSettings->CreateInputDiv(
                    '_tag_pulldown_first',
                    __('Tags First Entry', 'csa-slp-pro'),
                    __('The first entry on the search by tag pulldown.','csa-slp-es')
                    )
            ;
        ...
        $HTML .=
            "<div id='addy_in_city'>".
                "<select id='addressInput2' onchange='$onChange'>".
                    "<option value=''>".
                        get_option(SLPLUS_PREFIX.'_search_by_city_pd_label',__('--Search By City--','csa-slp-pro')).
                     '</option>'.
                    $this->create_CityPD().
                '</select>'.
            '</div>'
        ...
        $HTML .=
            "<div id='addy_in_country'>".
                "<select id='addressInput3' onchange='$onChange'>".
                    "<option value=''>".
                        get_option(SLPLUS_PREFIX.'_search_by_country_pd_label',__('--Search By Country--','csa-slp-pro')).
                     '</option>'.
        ...
        $HTML .=
            "<div id='addy_in_state'>".
                ...
                "<select id='addressInputState' onchange='$onChange'>".
                    "<option value=''>".
                        get_option(SLPLUS_PREFIX.'_search_by_state_pd_label',__('--Search By State--','csa-slp-pro')).
                     '</option>'.
                    $this->create_StatePD().
                '</select>'.
            '</div>'
    ...
    function action_EnqueueAdminScripts($hook) {
        $this->plugin->debugMP('slp.pro','msg',"Enqueue Admin Scripts for hook $hook");

        switch ($hook) {
            case 'slp-pro/reporting.php':
                wp_enqueue_script('jquery_tablesorter', $this->url  .'/jquery.tablesorter.js', array('jquery'             ));
                wp_enqueue_script('slp_reporting_js'  , $this->url  .'/reporting.js'         , array('jquery_tablesorter' ));
                $scriptData = array(
                    'plugin_url'        => SLPLUS_PLUGINURL,
                    'core_url'          => SLPLUS_COREURL,
                    );
                wp_localize_script('slp_reporting_js','slp_pro',$scriptData);
                $this->plugin->AdminUI->enqueue_admin_stylesheet();
               break;

            case 'store-locator-plus_page_slp_propack':
                $this->plugin->AdminUI->enqueue_admin_stylesheet();
                break;
    ...
    function renderPage_ProPack_Settings() {
        ...
        // General Settings
        //-------------------------
        $sectName = __('General Settings','csa-slp-pro');
        $this->ProPack_Settings->add_section(
            array(
                    'name'          => $sectName,
                    'description' =>
                        __('These settings affect how the Pro Pack add-on behaves. ', 'csa-slp-pro') .
                        '<span style="float:right;">(<a href="#" onClick="'.
                        'jQuery.post(ajaxurl,{action: \'license_reset_propack\'},function(response){alert(response);});'.
                        '">'.__('Delete license','csa-slp-pro').'</a>)</span>',
                    'auto'          => true
                )
         );
        ...
        $this->ProPack_Settings->add_item(
            $sectName,
            __('Enable reporting', 'csa-slp-pro'),
            'reporting_enabled',
            'checkbox',
            false,
            __('Enables tracking of searches and returned results.  The added overhead ' .
            'can increase how long it takes to return location search results.', 'csa-slp-pro')
        );
